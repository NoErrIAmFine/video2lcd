...
static int FBDeviceInit(void)
{
	...
	// g_tFBOpr.iBpp        = 16;			//驱动得出的数据有问题
	g_tFBOpr.iLineWidth  = g_tFBVar.xres * g_tFBOpr.iBpp / 8;
	g_dwLineWidth  = g_tFBVar.xres * g_tFBOpr.iBpp / 8;
	g_dwPixelWidth = g_tFBOpr.iBpp / 8;
	g_dwScreenSize = g_tFBOpr.iLineWidth * g_tFBVar.yres;
...
static int FBShowPage(PT_PixelDatas ptPixelDatas)
{
	// memset(g_tFBOpr.pucDispMem, 0xff, ptPixelDatas->iLineBytes);
    if (g_tFBOpr.pucDispMem != ptPixelDatas->aucPixelDatas)
    {
		printf("%s-%d-ptPixelDatas->iTotalBytes%d\n",__func__,__LINE__,ptPixelDatas->iTotalBytes);
